'''
b) Let us then update the visual appearance. Modify the 
code so that when the velocity of the particle exceeds the value 3.0,
it turns purple. Remember that the velocity can be calculated
 from the components using the formula v=v2x+v2y−−−−−−√.
 In Python, the square of any number a is denoted by a**2 while its square root is a**(1/2).

Hint: Add a conditional statement inside the function "update".
'''
import pygame
import math
